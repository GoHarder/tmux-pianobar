...
2. Use <kbd>prefix</kbd>–<kbd>I</kbd> install `tmux-pianobar`. You should now
   be able to `tmux-pianobar` immediately.
...
You should now be able to use `tmux-pianobar` immediately.

## Requirements

In order for `tmux-pianobar` to work, there must be a program that store data in
...
in `copy-mode` and mouse selection will work without `tmux-pianobar`.
...
`copy-mode` and mouse selection will work without `tmux-pianobar`. See the
...
tmux-pianobar does its best to detect a reasonable choice for a clipboard
program on your OS.

If tmux-pianobar can't detect a known clipboard program then it uses the
`@custom_copy_command` tmux option as your clipboard program if set.

If you need to always override tmux-pianobar's choice for a clipboard program,
then you can set `@override_copy_command` to force tmux-pianobar to use whatever
you want.
...
`clipboard` (default in tmux-pianobar is `clipboard`).
...
By default, `tmux-pianobar` will exit copy mode after yanking text. If you wish to
...
`tmux-pianobar` has mouse support enabled by default. It will only work if `tmux`'s
